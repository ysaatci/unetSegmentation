...
def dice_coef(y_true, y_pred, smooth=1):
    ...
    return (2. * intersection + smooth) / (keras.sum(keras.square(y_true), -1)
                                           + keras.sum(keras.square(y_pred),
                                                       -1) + smooth)
...
def unet(pretrained_weights = None,input_size = (256,256,1)):
    ...
    model.compile(optimizer = Adam(lr = 3e-5), loss = dice_coef_loss, metrics
                  = ['accuracy'])
